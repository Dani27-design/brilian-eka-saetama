...
export function generateHybridQRURL(
  ...
  const encodedData = Buffer.from(JSON.stringify(productData)).toString('base64');
  ...
  try {
    ...
    if (dataParam) {
      try {
        ...
        // Validate the data
        if (productData.productId && productData.productNumber && productData.productType) {
          return productData;
        }
      } catch (decodeError) {
...
export function isValidHybridQRURL(qrString: string): boolean {
  try {
    ...
    return url.pathname.includes('/product/') && url.pathname.includes('/certificates');
  } catch {
    return false;
  }
}
...
export function createHybridQRData(
  ...
  const url = generateHybridQRURL(product, productId, contractId, location, customerName, baseUrl);
