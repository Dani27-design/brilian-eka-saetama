/**
 * Tests for analytics overview API route: GET /api/analytics
 *
...
describe("GET /api/analytics — analytics overview endpoint", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe("success cases", () => {
    it("should return 200 with analytics data when admin is authenticated", async () => {
      // Arrange
      const req = new NextRequest(
        "http://localhost:3000/api/analytics?startDate=7daysAgo&endDate=today",
        {
      ...
      (getOverviewData as jest.Mock).mockResolvedValue(mockAnalyticsData);
      ...
      const body = await response.json();
      expect(body).toEqual(mockAnalyticsData);
      expect(getOverviewData).toHaveBeenCalledWith("7daysAgo", "today");
    });
    ...
    it("should use default date range when not provided", async () => {
      ...
      // Assert
      expect(response.status).toBe(200);
      expect(getOverviewData).toHaveBeenCalledWith("30daysAgo", "today");
    });
  });
  ...
  describe("error cases", () => {
    it("should return 500 when analytics service fails", async () => {
      ...
      (verifyAdminAuth as jest.Mock).mockResolvedValue(null);
      (getOverviewData as jest.Mock).mockResolvedValue(null);
      ...
      const body = await response.json();
      expect(body).toEqual({ error: "Failed to fetch analytics data" });
    });

    it("should return 500 when analytics service throws an exception", async () => {
      // Arrange
      const req = new NextRequest("http://localhost:3000/api/analytics", {
        headers: { Authorization: "Bearer valid-admin-token" },
      });
      ...
      (verifyAdminAuth as jest.Mock).mockResolvedValue(null);
      (getOverviewData as jest.Mock).mockRejectedValue(
