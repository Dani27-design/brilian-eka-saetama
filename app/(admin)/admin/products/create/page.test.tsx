/**
 * Tests for UI-CREATE: Create Product page visual alignment
 *
 * Validates that the create product page uses the unified admin card styling:
...
describe("Create Product Page — Visual Alignment", () => {
  ...
  it("should render without syntax errors (file is readable)", () => {
    ...
    expect(sourceCode).toContain("export default function CreateProductPage");
  });

  it("should have outer wrapper with flex h-full flex-col classes", () => {
    // Find the return statement's first div (outer wrapper)
    const returnMatch = sourceCode.match(/return\s*\(\s*<div\s+className="([^"]*)"/);
    expect(returnMatch).not.toBeNull();
    const outerClasses = returnMatch![1];
