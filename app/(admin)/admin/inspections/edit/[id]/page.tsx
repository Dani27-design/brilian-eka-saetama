...
export default function EditInspectionPage() {
  ...
  const { user } = useAdmin();
  usePageHeader("Edit Inspeksi", "Ubah data hasil inspeksi dan kelola foto");
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-row w-full justify-between mb-4">
        ...
        {/* Meta Information — show the first (most recent) audit trail entry */}
        {auditTrail.length > 0 && (
          <div className="inline-flex h-10 items-center rounded-lg border border-stroke bg-white px-4 text-xs text-blue-800">
            <span className="font-medium">{auditTrail[0].label} : </span>
            <span className="ml-1">
              <b>{auditTrail[0].user.name}</b>
              {auditTrail[0].user.role ? ` (${auditTrail[0].user.role})` : ""}
            </span>
            <span className="ml-1 text-blue-600">{auditTrail[0].date}</span>
          </div>
        )}
      </div>
      ...
      {/* Photo Modal — keep outside scroll area, EXACTLY AS-IS */}
      <Modal
